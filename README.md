...
> **Interactive Opening Repertoire Trainer** — Master theoretical chess lines with structured active recall, visual ambiguity cues, move-by-move master annotations, and progressive multi-mode training decks.
...
**Modena Lines** is a high-performance, web-based chess opening repertoire trainer designed to bridge the gap between passive memorization and active master-level recall. Built on a tactile **neumorphic design system**, Modena Lines presents opening theory through intuitive board visualization, dynamic branching prompts, and graduated practice decks.
...
### 5. Human-Paced Move & Animation Engine
...
| **Logic & State** | Vanilla Modern JavaScript (ES Modules, LocalStorage persistence engine) |
...
4. **Build production bundle for local inspection:**
   ```bash
   npm run build
   npm run preview
   ```
...
│   ├── storage/
│   │   └── user-progress.js  # Accuracy, streak, and local storage state
│   ├── ui/
...
│   ├── components.css        # Cards, badges, buttons, and mode deck styling
