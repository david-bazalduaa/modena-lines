...
export function renderSubCourseHub(course, userProgress, onSelectSubCourse, onBackToCatalog) {
  ...
  try {
    ...
    $('#subcourse-hero-title').text(course.title || 'Course Modules');
    ...
    subCourses.forEach(sub => {
      ...
      lines.forEach(line => {
        if (!line) return;
        try {
          if (userProgress && typeof userProgress.isLineCompleted === 'function' ? userProgress.isLineCompleted(line) : (userProgress && userProgress.getLineStat(line.id).completed)) {
            totalCourseMastered++;
          }
        } catch (e) {
          // ignore
    ...
    subCourses.forEach((subCourse) => {
      try {
        const lines = Array.isArray(subCourse.lines) ? subCourse.lines : [];
        ...
        let miniBoardHTML = '';
        try {
          miniBoardHTML = generateMiniBoardHTML(subCourse.previewFEN);
        } catch (e) {
        ...
        const cardHTML = `
          <div class="course-card subcourse-card" data-subcourse-id="${subCourse.id || ''}">
            ...
            <div class="course-card-info">
              <h4>${subCourse.title || 'Sub-Course Module'}</h4>
        ...
        console.error('[SubCourseView] Error rendering subcourse card:', subErr);
      }
    });
    ...
    $('.view-subcourse-btn, .subcourse-card').off('click').on('click', function (e) {
      ...
      const matchedSub = subCourses.find(s => s && s.id === subId);
      if (matchedSub && onSelectSubCourse) {
        onSelectSubCourse(matchedSub);
      }
    });
  } catch (err) {
    console.error('[SubCourseView] Hub render failed:', err);
  }
}
